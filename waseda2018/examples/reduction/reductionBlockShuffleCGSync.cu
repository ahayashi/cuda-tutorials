...
__inline__ __device__ int tile_reduce(cg::thread_block_tile<32> g, int val)
{
...
__inline__ __device__ int blockReduceSumCG(int val) {
    ...
    int wid = threadIdx.x / g.size();

    // per warp reduction    
    int valPerWarp = tile_reduce(g, val);

    // Write reduced value to shared memory
    if (lane == 0) shared[wid] = valPerWarp;
    ...
    if (wid==0) val = tile_reduce(g, val);
...
__global__ void reduce(int *A, int *sum, int N)
{
    ...
    int valPerBlock = blockReduceSumCG(val);      
